...
function h(tag, customElement = "", attrs, ...children) {
  const el = customElement.length > 0 ? 
    document.createElement(tag, {is: customElement}) : 
...
class BenchRow extends HTMLTableRowElement {
  render(rowId, label) {
    const idCell = h('td', 'bench-cell', {class: 'col-md-1'}, rowId);
    
    this.labelLink = h('a', '', {onclick: (e) => {
      e.stopPropagation();
      this.select();
    ...
    const labelCell = h('td', 'bench-cell', {class: 'col-md-4'}, this.labelLink);

    const icon = h('span', 'bench-icon', {});
    const closeLink = h('a', '', {onclick: (e) => {
      e.stopPropagation();
      this.remove();
    }}, icon);
    const closeCell = h('td', 'bench-cell', {class: 'col-md-1'}, closeLink);
    
    const emptyCell = h('td', 'bench-cell', {class: 'col-md-6'});
  ...
  addToLabel(content) {
    this.labelLink.textContent += content;
  }
}
